...
language:
  ...
  title-block-correspondence-note: "Correspondence concerning this article should be addressed to Lisa."
...
```{r Inspect Dataset, results='hide'}
...
```{r Prepare dataset for visualization}
...
```{r Table for Descriptive Data Analysis 1}
...
```{r Graph Descriptive Analysis 1}
...
```{r Graph Descriptive Data Analysis 2}
...
```{r Does weight predict mpg? Does having v-shaped or straight engine change this prediction?}
...
plot7
```
```{r Compare results example}
#| label: Compare resultsusing include_graphics()
#| echo: TRUE
